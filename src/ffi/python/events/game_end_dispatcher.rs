...
pub(crate) trait GameEndDispatcherMethods<'py> {
    fn dispatch(&self, data: &Bound<'py, PyAny>) -> PyResult<Bound<'py, PyAny>>;
}

impl<'py> GameEndDispatcherMethods<'py> for Bound<'py, GameEndDispatcher> {
    fn dispatch(&self, data: &Bound<'py, PyAny>) -> PyResult<Bound<'py, PyAny>> {
...
mod game_end_dispatcher_tests {
    ...
    fn dispatch_with_no_handlers_registered(_pyshinqlx_setup: ()) {
        Python::with_gil(|py| {
            let dispatcher =
                Bound::new(py, GameEndDispatcher::py_new(py)).expect("this should not happen");
    ...
    fn dispatch_when_handler_returns_exception(_pyshinqlx_setup: ()) {
        ...
        MockEngineBuilder::default()
            ...
            .run(|| {
                Python::with_gil(|py| {
                    let dispatcher = Bound::new(py, GameEndDispatcher::py_new(py))
                        .expect("this should not happen");
