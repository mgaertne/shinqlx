...
pub(crate) trait RoundStartDispatcherMethods<'py> {
    fn dispatch(&self, round_number: i32) -> PyResult<Bound<'py, PyAny>>;
}

impl<'py> RoundStartDispatcherMethods<'py> for Bound<'py, RoundStartDispatcher> {
    fn dispatch(&self, round_number: i32) -> PyResult<Bound<'py, PyAny>> {
        Ok(self
            .as_super()
...
#[cfg(test)]
mod round_start_dispatcher_tests {
    use super::{RoundStartDispatcher, RoundStartDispatcherMethods};
