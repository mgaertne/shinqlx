...
mod vote_called_dispatcher_tests {
    use super::VoteCalledDispatcher;
    ...
    fn dispatch_with_no_handlers_registered(_pyshinqlx_setup: ()) {
        Python::with_gil(|py| {
            let dispatcher =
                Py::new(py, VoteCalledDispatcher::py_new(py)).expect("this should not happen");
    ...
    fn dispatch_when_handler_returns_exception(_pyshinqlx_setup: ()) {
        ...
        mocked_engine()
            ...
            .run(|| {
                Python::with_gil(|py| {
                    let dispatcher = Py::new(py, VoteCalledDispatcher::py_new(py))
                        .expect("this should not happen");
