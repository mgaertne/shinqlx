...
pub(crate) trait KamikazeUseDispatcherMethods<'py> {
    fn dispatch(&self, player: &Bound<'py, Player>) -> PyResult<Bound<'py, PyAny>>;
}

impl<'py> KamikazeUseDispatcherMethods<'py> for Bound<'py, KamikazeUseDispatcher> {
    fn dispatch(&self, player: &Bound<'py, Player>) -> PyResult<Bound<'py, PyAny>> {
        let args_tuple = PyTuple::new(self.py(), [player])?;
...
mod kamikaze_use_dispatcher_tests {
    ...
    fn dispatch_with_no_handlers_registered(_pyshinqlx_setup: ()) {
        Python::with_gil(|py| {
            let dispatcher =
                Bound::new(py, KamikazeUseDispatcher::py_new(py)).expect("this should not happen");
    ...
    fn dispatch_when_handler_returns_exception(_pyshinqlx_setup: ()) {
        ...
        MockEngineBuilder::default()
            ...
            .run(|| {
                Python::with_gil(|py| {
                    let dispatcher = Bound::new(py, KamikazeUseDispatcher::py_new(py))
                        .expect("this should not happen");
