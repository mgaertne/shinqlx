...
impl VoteDispatcher {
    ...
    fn dispatch(slf: PyRef<'_, Self>, py: Python<'_>, player: PyObject, yes_no: bool) -> bool {
        let mut return_value = true;

        let super_class = slf.into_super();
        if let Ok(logger) = pyshinqlx_get_logger(py, None) {
            if let Ok(player_str) = player.call_method0(py, intern!(py, "__str__")) {
                let mut dbgstr = format!("{}({}, {})", super_class.name, player_str, yes_no);
                if dbgstr.len() > 100 {
                    dbgstr.truncate(99);
                    dbgstr.push(')');
                }
        ...
        for i in 0..5 {
            for (_, handlers) in &super_class.plugins {
                for handler in &handlers[i] {
                    match handler.call1(py, (&player, yes_no)) {
                        Err(e) => {
                            log_exception(py, e);
                            continue;
                        }
