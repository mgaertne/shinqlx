...
pub(crate) trait StatsDispatcherMethods<'py> {
    fn dispatch(&self, data: &Bound<'py, PyAny>) -> PyResult<Bound<'py, PyAny>>;
}

impl<'py> StatsDispatcherMethods<'py> for Bound<'py, StatsDispatcher> {
    fn dispatch(&self, data: &Bound<'py, PyAny>) -> PyResult<Bound<'py, PyAny>> {
...
mod stats_dispatcher_tests {
    use super::{StatsDispatcher, StatsDispatcherMethods};
    ...
    fn dispatch_with_no_handlers_registered(_pyshinqlx_setup: ()) {
        Python::with_gil(|py| {
            let dispatcher =
                Bound::new(py, StatsDispatcher::py_new(py)).expect("this should not happen");

            let result = dispatcher.dispatch(&PyString::new(py, "asdf"));
    ...
    fn dispatch_when_handler_returns_exception(_pyshinqlx_setup: ()) {
        ...
        MockEngineBuilder::default()
            ...
            .run(|| {
                Python::with_gil(|py| {
                    let dispatcher = Bound::new(py, StatsDispatcher::py_new(py))
                        .expect("this should not happen");
