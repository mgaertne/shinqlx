...
pub(crate) trait PlayerSpawnDispatcherMethods<'py> {
    fn dispatch(&self, player: &Bound<'py, Player>) -> PyResult<Bound<'py, PyAny>>;
}

impl<'py> PlayerSpawnDispatcherMethods<'py> for Bound<'py, PlayerSpawnDispatcher> {
    fn dispatch(&self, player: &Bound<'py, Player>) -> PyResult<Bound<'py, PyAny>> {
        Ok(self
            .as_super()
...
mod player_spawn_dispatcher_tests {
    use super::{PlayerSpawnDispatcher, PlayerSpawnDispatcherMethods};
    ...
    fn dispatch_with_no_handlers_registered(_pyshinqlx_setup: ()) {
        Python::with_gil(|py| {
            let dispatcher =
                Bound::new(py, PlayerSpawnDispatcher::py_new(py)).expect("this should not happen");
    ...
    fn dispatch_when_handler_returns_exception(_pyshinqlx_setup: ()) {
        ...
        MockEngineBuilder::default()
            ...
            .run(|| {
                Python::with_gil(|py| {
                    let dispatcher = Bound::new(py, PlayerSpawnDispatcher::py_new(py))
                        .expect("this should not happen");
