...
pub(crate) trait UnloadDispatcherMethods<'py> {
    fn dispatch(&self, plugin: &Bound<'py, PyAny>) -> PyResult<Bound<'py, PyAny>>;
}

impl<'py> UnloadDispatcherMethods<'py> for Bound<'py, UnloadDispatcher> {
    fn dispatch(&self, plugin: &Bound<'py, PyAny>) -> PyResult<Bound<'py, PyAny>> {
        Ok(self
            .as_super()
            .dispatch(&PyTuple::new(self.py(), [plugin])?))
    }
}

#[cfg(test)]
mod unload_dispatcher_tests {
    use super::{UnloadDispatcher, UnloadDispatcherMethods};
